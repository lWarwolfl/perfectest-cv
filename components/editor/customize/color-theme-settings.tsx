...
const PRESET_ACCENTS = ['#044cb5', '#0891b2', '#ea580c', '#4a7c59', '#b91c1c', '#475569', '#1e293b', '#7c3aed']
...
export default function ColorThemeSettings({
  ...
  const setSingle = (hex: string) =>
    onPatch({ mode: 'basic', basic: { ...basic, single: hex, singleCustom: hex, selected: 'single' } })

  return (
    <CustomizeCard title="Colors" icon={Palette} description="Accent color applied across the document.">
        ...
        {PRESET_ACCENTS.map((hex) => (
          <button
            ...
            className={`flex size-8 cursor-pointer items-center justify-center rounded-full border-2 transition-transform hover:scale-110 ${
              basic.single === hex ? 'ring-2 ring-primary ring-offset-2' : ''
            }`}
      ...
      {customOpen && (
        <div className="space-y-2">
          <HexField label="Accent" value={basic.singleCustom} onChange={(hex) => onPatch({ mode: 'basic', basic: { ...basic, single: hex, singleCustom: hex, selected: 'single' } })} />
          <HexField label="Text" value={basic.multi.textColor} onChange={(hex) => onPatch({ mode: 'basic', basic: { ...basic, selected: 'multi', multi: { ...basic.multi, textColor: hex } } })} />
          <HexField label="Accent (multi)" value={basic.multi.accentColor} onChange={(hex) => onPatch({ mode: 'basic', basic: { ...basic, selected: 'multi', multi: { ...basic.multi, accentColor: hex } } })} />
