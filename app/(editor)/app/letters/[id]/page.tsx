...
export default function LetterEditorPage() {
  ...
  function patchCustomization(patch: Partial<Customization>) {
    setDesign((d) =>
      normalizeLetterDesign({
        ...(d || undefined),
        customization: { ...(d?.customization || ({} as Customization)), ...patch },
      })
    )
    markDirty()
  }
  ...
  const sections: { key: SectionKey; badge?: string; preview: string; placeholder?: boolean }[] = [
    {
      key: 'sender',
      badge: form.senderPhotoImageId ? 'With photo' : undefined,
      preview: [form.senderName, form.senderJobTitle].filter(Boolean).join(' • ') || 'Not added',
    },
    { key: 'date', preview: form.dateMode === 'custom' ? form.dateCustom || 'Not added' : today },
    {
      key: 'recipient',
      preview:
        [form.recipientName, form.recipientCompany].filter(Boolean).join(' • ') || 'Not added',
    },
    { key: 'subject', preview: form.subject || 'Not added', placeholder: !form.subject },
    {
      key: 'body',
      badge: `${(form.body || '').split(/\s+/).filter(Boolean).length} words`,
      preview:
        (form.body || '')
          .replace(/<[^>]*>/g, ' ')
          .trim()
          .slice(0, 80) || 'Not added',
            ...
            <div className="bg-background sticky top-0 z-10 flex items-center gap-2 border-b p-3">
              <Button
                ...
                onClick={() =>
                  activeSection
                    ? closeSection(false)
                    : setTab(tab === 'content' ? 'design' : 'content')
                }
              ...
              <Button
                size="sm"
                disabled={!titleDraft.trim() || titleDraft === (letter?.title || '')}
                onClick={() => {
              ...
              activeSection ? (
                <div className="flex min-h-0 flex-1 flex-col">
                  <div className="flex items-center gap-2 border-b p-3">
                    ...
                    <Button size="sm" onClick={() => closeSection(true)}>
                      Save
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => closeSection(false)}>
                      Cancel
                    </Button>
                  </div>
                  <div className="min-h-0 flex-1 overflow-y-auto p-3">
                    ...
                    {activeSection === 'recipient' && (
                      <RecipientDetailsForm value={form} onChange={patchForm} />
                    )}
                    {activeSection === 'subject' && (
                      <SubjectForm value={form} onChange={patchForm} />
                    )}
                    {activeSection === 'body' && <BodyForm value={form} onChange={patchForm} />}
                    {activeSection === 'signature' && (
                      <SignatureForm value={form} onChange={patchForm} />
                    )}
                  </div>
                </div>
              ) : (
                    ...
                    {sections.map((s) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p
                            className={`mt-0.5 line-clamp-1 pr-4 text-xs ${s.placeholder ? 'text-muted-foreground/60 italic' : 'text-muted-foreground'}`}
                          >
                            {s.preview}
                          </p>
                        </div>
