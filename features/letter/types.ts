...
export function normalizeLetterDesign(saved?: LetterDesign | LegacyDesign | null): LetterDesign {
  ...
  const out: LetterDesign = {
    ...base,
    ...(s.letterDateDisplay?.position
      ? { letterDateDisplay: { position: s.letterDateDisplay.position } }
      : {}),
  ...
  if (s.fontSizePt !== undefined)
    c.spacing = { ...c.spacing, fontSize: String(clamp(s.fontSizePt - 10, 0, 8)) }
  if (s.lineHeightPct !== undefined)
    c.spacing = {
      ...c.spacing,
      lineHeight: String(clamp(Math.round((s.lineHeightPct - 1.2) * 10), 1, 5)),
    }
  if (s.nameFontSizePt !== undefined) c.spacing = { ...c.spacing, nameFontSizePt: s.nameFontSizePt }
  if (s.jobTitleFontSizePt !== undefined)
    c.spacing = { ...c.spacing, jobTitleFontSizePt: s.jobTitleFontSizePt }
  if (s.verticalMarginMm !== undefined)
    c.spacing = {
      ...c.spacing,
      marginVertical: String(clamp(Math.round((s.verticalMarginMm - 10) / 3), 0, 6)),
    }
  if (s.horizontalMarginMm !== undefined)
    c.spacing = {
      ...c.spacing,
      marginHorizontal: String(clamp(Math.round((s.horizontalMarginMm - 10) / 3), 0, 6)),
    }
  if (s.headerSettings) c.header = { ...c.header, ...s.headerSettings }
  if (s.colors?.basic?.single)
    c.colors = { ...c.colors, basic: { ...c.colors.basic, single: s.colors.basic.single } }
  out.customization = c
  return out
}
